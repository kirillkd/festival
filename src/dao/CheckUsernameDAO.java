...
public class CheckUsernameDAO extends DAO {
	...
	public void getCheckUsername(VisitorAccountBean visitor_account, ArrayList<BandBean> bands) throws RuntimeException, SQLException, ClassNotFoundException {
		...
		pstmt2.setString(2, visitor_account.getUsername());
		
		ResultSet rs1 = pstmt1.executeQuery();
		ResultSet rs2 = pstmt2.executeQuery();
		
		if(rs1.next()==false) {
			throw new RuntimeException("Username " + visitor_account.getUsername() + " does not exist!");
		} else{
			while(rs2.next()) {
